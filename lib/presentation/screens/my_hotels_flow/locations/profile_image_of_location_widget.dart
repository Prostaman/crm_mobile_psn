...
Widget? profileImageOfLocation(int index, LocationModel location,
    List<List<FileModel>?> listOflistOfFiles) {
  if (location.pathOfProfilePhoto.isNotEmpty &&
      File(location.pathOfProfilePhoto).existsSync()) {
    ...
    for (var file in listOflistOfFiles[index]!) {
      //find first image that not deleted and exists
