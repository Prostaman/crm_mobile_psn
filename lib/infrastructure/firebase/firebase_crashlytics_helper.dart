...
class FirebaseCrashlyticsHelper {
  static Future<void> recordApiError(
      BaseModelResponse? response, String nameOfMethod) async {
    if (response != null) {
      var errors = response.errors;
      for (int index = 0; index < errors.length; index++) {
        String error =
            "API: $nameOfMethod error number: $index\nerror code: ${errors[index].code}\nerror message: ${errors[index].message}";
        debugPrint(error);
  ...
  static Future<void> recordDaoLocalDBError(
      String e, String nameOfMethod) async {
