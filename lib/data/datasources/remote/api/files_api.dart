...
import 'package:psn.hotels.hub/data/datasources/remote/api/base_api.dart';
...
class FilesApi extends BaseApi {
  Future<FileResponse?> send({
    ...
    try {
      FormData data = FormData.fromMap({
        ...
        "file": await MultipartFile.fromFile(file.localPath,
            filename: file.name //, contentType: new MediaType("image", "jpeg")
            )
      });
      ...
      await FirebaseCrashlytics.instance
          .log("send file:$e, localPath:${file.localPath}");
      await FirebaseCrashlytics.instance
  ...
  Future<BaseModelResponse?> changeProfilePhoto(
      {required int cloudId, required bool isHotel}) async {
    try {
      var response = await super
          .post("UserHotelSetMainMedia", {"id": cloudId, 'ishotel': isHotel});
      Map<String, dynamic> jsonMap = json.decode(response.toString());
      var model = BaseModelResponse.fromJson(jsonMap);
      return model;
    } catch (e) {
